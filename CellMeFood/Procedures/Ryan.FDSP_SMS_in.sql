...
-- =============================================
CREATE PROCEDURE [Ryan].[FDSP_SMS_in] 
(
...
declare @isadmin bit, @sessionid nvarchar(100), @isOnly bit, @id int
set @questtest=0
--set @string = ' -yams +corn +chicken -mushroom'
declare @countries Type_NamesList	-- DP: countries filter table
declare @meats Type_IdsList			-- DP: meats filter table
...
-- DP: re-order words table in desirable way:
select @origlen = max(id) from @words		-- DP: get last element
;

insert @words (element) 
	select element from @words				-- DP: re-sort elements list the right way
	order by case left(element,1)
		when '?' then 1
		when '+' then 2
		when '-' then 3
		else 99 end
...
delete @words where id <= @origlen			-- DP: remove original part of list
;
...
set @id = 0; while (1=1) -- @origlen > 0                                  ---------------iterate through input loop
begin

	-- DP: get words one by one until we are done
	--set @input = (select top 1 element from @words)
	select top 1 @input = element, @id = id 
		from @words where id > @id order by id
	;
	...
	begin
		...
		begin
			...
			begin
				...
				-- DP: handle meats
				insert @meats
					select recipeid from MeattoRecipe mr 
					inner join Meats m on m.id = mr.meatid
					where m.meatname = @input
					and not exists(select Id from @meats where Id=recipeid)
				;
			...
			begin
				...
				delete x
					from @meats x
					inner join MeattoRecipe mr on mr.recipeid = x.Id
					inner join Meats m on m.id = mr.meatid
					where m.meatname = @input
				;
...
Optional category narrows down items as desired.
...
add  - adds ingredients to shopping list
list - lists ingredients from shopping list
...
' 
		...
		begin
			...
			begin
				insert into list(userid,recipeid,ingrediantid,ingrediantorder,ingredient) 
					select @userid,0,0,0,replace(replace(element,'+',''),'-','') from @words
					where element like '[+-]%'
				;
				select status1 = 'Added ' 
					+ convert(varchar(20),(select count(1) from @words where element like '[+-]%')) 
					+ ' ingredients'
					;
				return 0;
			end
			...
			begin
				select status1 = left(ingredient,120) from Ryan.List 
					where userid = @userid
					and	added = 0
					order by id
					;
...
set @sqlfinal=@sqlfinal+'; if not exists(select 1 from #temp) select top 1 word as status1 from [Ryan].[Reasons] order by newid()  else select top 5 status1 from #temp order by newid();'
...
--begin
	...
	set @sqlfinal = replace(@sqlfinal, N'{meats}', case when @isOnly=1 --and exists(select 1 from @meats) 
		then N'inner join @meats m on m.Id = r.Id'
		else N'/* all meats */' end
		);

	if @debug = 1 select @sqlfinal as sqlfinal

	--exec(@sqlfinal)
	exec sp_executesql @sqlfinal
	,	N'	@countries Type_NamesList readonly
		,	@meats Type_IdsList readonly'
	,	@countries
	,	@meats
	;
